...
-- Dumping data for table siakad_adhiguna_1.tb_dosen: ~2 rows (approximately)
/*!40000 ALTER TABLE `tb_dosen` DISABLE KEYS */;
REPLACE INTO `tb_dosen` (`nidn`, `nm_dosen`, `program_studi`) VALUES
	(12345, 'iank', 55201),
	(243552, 'Zainal Arifin', 55201),
	(5520111, 'Siap', 55201);
...
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=latin1 COMMENT='get data dari tb_mhs_data_krs berdasarkan kurikulum yang diambil dari field id_kelas kemudian isi tabel ini';

-- Dumping data for table siakad_adhiguna_1.tb_kelas_dosen: ~3 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `tb_kelas_kul` (
  ...
  CONSTRAINT `FK_tb_kelas_kul_tb_matkul` FOREIGN KEY (`id_matkul`) REFERENCES `tb_mk_kurikulum` (`kode_mk`) ON UPDATE CASCADE,
  CONSTRAINT `FK_tb_kelas_kul_tb_prodi` FOREIGN KEY (`id_prodi`) REFERENCES `tb_prodi` (`id_prodi`) ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=18 DEFAULT CHARSET=latin1 COMMENT='get prodi dan get mata kuliah berdasarkan kurikulum sebelum memasukan data, || seharusnya bukan id_mk_kur bukan id_matkul di tabel mk_kurikulum';

-- Dumping data for table siakad_adhiguna_1.tb_kelas_kul: ~11 rows (approximately)
...
REPLACE INTO `tb_kelas_kul` (`id_kelas`, `nm_kelas`, `id_matkul`, `id_kurikulum`, `id_prodi`) VALUES
	...
	(12, '1', 'ST 117 IN2', 2, 55201),
	(13, '1', 'ST 111 IN2', 3, 57201),
	(14, '2', 'ST 111 IN2', 3, 57201),
	(15, '3', 'ST 111 IN2', 3, 57201),
	(17, '2', 'ST 117 IN2', 2, 55201);
...
REPLACE INTO `tb_kurikulum` (`id_kurikulum`, `nm_kurikulum`, `ta`, `kd_prodi`, `status`) VALUES
	...
	(5, 'STMIK Adhi Guna TI-20161', '20161', 55201, '0'),
	(6, 'STMIK Adhi Guna SI-20161', '20161', 57201, '0');
...
-- Dumping data for table siakad_adhiguna_1.tb_mata_kuliah: ~3 rows (approximately)
/*!40000 ALTER TABLE `tb_mata_kuliah` DISABLE KEYS */;
REPLACE INTO `tb_mata_kuliah` (`kode_mk`, `nm_mk`, `sks`, `semester`) VALUES
	('12345', 'ALGORITMA PEMROGRAMAN', 3, 1),
	('ST 108 IN3', 'STATISK LANJUT', 3, 2),
	('ST 111 IN2', 'BAHASA INGGRIS 2', 2, 2),
	('ST 117 IN2', 'ETIKA PROFESI', 2, 2),
	('ST 119 UT2', 'LOGIKA INFORMATIKA', 3, 2);
...
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=latin1 COMMENT='get id krs dari tb_mhs_krs kemudian get kelas berdasarkan kurikulum dari tb_mhs_krs ';

-- Dumping data for table siakad_adhiguna_1.tb_mhs_data_krs: ~3 rows (approximately)
...
REPLACE INTO `tb_mhs_krs` (`id_krs`, `id_mhs`, `kode_pembayaran`, `id_kurikulum`, `status_ambil`, `status_cek`) VALUES
	(1, '5520111008', '897103d1e884257bff412c565ada8a5f', 2, 'T', 'Y'),
...
REPLACE INTO `tb_mhs_lulus` (`id_mhs`, `id_jns_keluar`, `tgl_keluar`, `jalur_skripsi`, `judul_skripsi`, `bln_awal_bim`, `bln_akhir_bim`, `sk_yudisium`, `tgl_yudisium`, `IPK`, `no_ijazah`, `ket`) VALUES
	('5520111008', 1, '2015-10-30', '1', 'IMPLEMENTASI RANDOM SERVIS UNTUK MESIN ANTRIAN BERBASIS RASPBERRY PI PADA WARKOP RICKY', '2015-09-30', '2015-10-30', 'bla', '2015-10-30', '4.00', 'sdv', 'sdvsd'),
...
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=latin1 COMMENT='Sdh Input';
...
REPLACE INTO `tb_mhs_transfer` (`id_trans`, `id_mhs`, `sks_diakui`, `pt_asal`, `prodi_asal`) VALUES
	...
	(2, '5520115061', 89, 'AMIK TRI DHARMA PALU', 'Teknik Informatika');
...
-- Dumping data for table siakad_adhiguna_1.tb_mk_kurikulum: ~2 rows (approximately)
/*!40000 ALTER TABLE `tb_mk_kurikulum` DISABLE KEYS */;
REPLACE INTO `tb_mk_kurikulum` (`id_kur_mk`, `kode_mk`, `id_kurikulum`) VALUES
	(1, 'ST 108 IN3', 2),
	(2, 'ST 111 IN2', 2),
	(3, 'ST 117 IN2', 2);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_kelas_dosen` AS select `m1`.`id_kelas_dosen` AS `id_kelas_dosen`,`m5`.`nidn` AS `nidn`,`m5`.`nm_dosen` AS `nm_dosen`,`m4`.`kode_mk` AS `kode_mk`,`m4`.`nm_mk` AS `nm_mk`,`m3`.`nm_kelas` AS `nm_kelas`,`m1`.`r_t_muka` AS `r_t_muka`,`m1`.`t_muka` AS `t_muka`,`m6`.`ta` AS `ta`,`m7`.`id_prodi` AS `id_prodi`,`m7`.`nm_prodi` AS `nm_prodi` from ((((((`tb_kelas_dosen` `m1` join `tb_mhs_data_krs` `m2` on((`m1`.`id_data_krs` = `m2`.`id_data_krs`))) join `tb_kelas_kul` `m3` on((`m2`.`id_kelas` = `m3`.`id_kelas`))) join `tb_mata_kuliah` `m4` on((`m3`.`id_matkul` = `m4`.`kode_mk`))) join `tb_dosen` `m5` on((`m1`.`id_dosen` = `m5`.`nidn`))) join `tb_kurikulum` `m6` on((`m3`.`id_kurikulum` = `m6`.`id_kurikulum`))) join `tb_prodi` `m7` on((`m3`.`id_prodi` = `m7`.`id_prodi`)));
