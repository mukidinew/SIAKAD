...
class Mata_kuliah_kurikulum extends CI_Controller
{
    ...
    public function getKurikulum(){
      ...
      $page = $this->input->post('page');
      if ($page=='') {
        $kurikulum = $this->app_model->get_query("SELECT * FROM tb_kurikulum WHERE kd_prodi='".$this->session->userdata('level_prodi') ."' ORDER BY ta DESC")->result();
      }
      else {
        //$kurikulum = $this->app_model->get_limit_kurikulum('tb_kurikulum',$page,0,$cari,$this->session->userdata('level_prodi'));
