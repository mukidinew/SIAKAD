...
class Nilai extends CI_Controller
{
    ...
    {
        $nilai = $this->app_model->get_all_view_nilai($this->session->userdata('level_prodi'));
    ...
    public function get_data_krs($value='')
    {
      ...
      if ($page=='') {
        $data_krs = $this->app_model->get_query("SELECT * FROM v_data_krs WHERE id_prodi='".$this->session->userdata('level_prodi')."'")->result();
      }
      else {
        $data_krs = $this->app_model->get_limit_data_krs('v_data_krs',$page,0,$cari,$this->session->userdata('level_prodi'));
      }
